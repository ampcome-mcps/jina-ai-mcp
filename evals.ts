...
const read_webpageEval: EvalFunction = {
    ...
    run: async () => {
        const result = await grade(openai("gpt-4"), "Extract the main content from https://example.com");
...
const search_web: EvalFunction = {
    ...
    run: async () => {
        const result = await grade(openai("gpt-4"), "What are the top search results for 'best pizza in NYC' using Jina AI's search API?");
        return JSON.parse(result);
    }
};
...
const fact_checkEval: EvalFunction = {
    ...
    run: async () => {
        const result = await grade(openai("gpt-4"), "Is it true that the Great Wall of China is visible from space?");
        return JSON.parse(result);
    }
};
